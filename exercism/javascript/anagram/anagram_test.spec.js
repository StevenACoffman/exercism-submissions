/*global it:false, it:false, expect:false, describe:false */
...
describe('Anagram', function () {
  ...
  it('reversed one word matches', function () {
    var subject = anagram('so');
    var matches = subject.matches(['os']);

    expect(matches).toEqual(['os']);
  });

  it('word does not match itself ', function () {
    var subject = anagram('person');
    var matches = subject.matches(['person']);

    expect(matches).toEqual([]);
  });

  it('one word matches', function () {
    var subject = anagram('cat');
    var matches = subject.matches(['act', 'dog']);

    expect(matches).toEqual(['act']);
  });
